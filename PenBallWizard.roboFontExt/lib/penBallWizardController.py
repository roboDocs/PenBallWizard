...
class PenBallWizardController(object):
    ...
    def generateGlyphsToFont(self, exportFont=None, layerName=None):
        ...
        if currentFont is not None and len(currentFont.selectedGlyphs):
            glyphs = [currentFont[glyphName] for glyphName in currentFont.selectedGlyphNames if glyphName in currentFont]
            for glyph in glyphs:
    ...
    def generateGlyphsToLayer(self, font, layerName):
        ...
        currentFilter = self.filters[filterName]
        if font is not None:
            glyphs = [font[glyphName] for glyphName in font.selectedGlyphNames if glyphName in font]
            for glyph in glyphs:
                if len(glyph.components) == 0:
    ...
    def filterGlyphs(self, filterName, glyphs, font):
        ...
        for glyph in glyphs:
            if len(glyph.components) > 0:
                for comp in glyph.components:
                    baseGlyphName = comp.baseGlyph
                    baseGlyph = glyph.font[baseGlyphName]
                    baseFilteredGlyph = currentFilter(baseGlyph)
    ...
    def buildFilterSheet(self, filterName='', makeNew=False):
        ...
        filePathTab.fileInput = SquareButton((-100, 10, 90, -10), 'Add File…', sizeStyle='small', callback=self.getFile)
        y += 75
    ...
    def buildFilterGroupSheet(self, filterName='', makeNew=False):
        ...
        y += 75
        self.filterSheet.moveSubfilterUp = SquareButton((15, y, buttonSize, buttonSize), '⇡', sizeStyle='small', callback=self.moveSubfilterUp)
        self.filterSheet.moveSubfilterDown = SquareButton((15, y + buttonSize + gutter, buttonSize, buttonSize), '⇣', sizeStyle='small', callback=self.moveSubfilterDown)
